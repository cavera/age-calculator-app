...
export const useDateCalc = ({ day, month, year }) => {
	...
	useEffect(() => {
		...
		const inputDay = day === undefined ? today.getUTCDate() : day
		const inputMonth = month === undefined ? today.getUTCMonth() : month
		const inputYear = year === undefined ? today.getUTCFullYear() : year
